...
def cosine_scores(q_vector: T, ctx_vectors: T):
    # q_vector: n1 x D, ctx_vectors: n2 x D, result n1 x n2
    # print(q_vector.shape, ctx_vectors.shape)
    q_vector = F.normalize(q_vector, p =2)
    ctx_vectors = F.normalize(ctx_vectors, p = 2)
    return torch.matmul(q_vector, torch.transpose(ctx_vectors, 0, 1))
...
class BiEncoder(nn.Module):
    ...
    def forward(
        ...
        output = self.linear(self.dropout(q_pooled_out))
        ...
        # sep_ids = [(ten == 102).nonzero()[0] for ten in question_ids]
        # null_vect = torch.zeros_like(torch.tensor(sep_ids))
        # for i, mask in zip(sep_ids, question_attn_mask):
        #         mask[i+1:] = 0
        
        # _q_seq2, q_pooled_out2, _q_hidden2 = self.get_representation(
        #     q_encoder,
        #     question_ids,
        #     question_segments,
        #     question_attn_mask,
        ...
        # q_pooled_out --> [P {SEP} H]
        # output = Linea(q_pooled_output)
        # q_pooled_out2 --> [P]

        return q_pooled_out, ctx_pooled_out  , output
...
class BiEncoderNllLoss(object):
    def calc(
        ...
        # q_vectors  -> [ P ] -> CLS
        # label -> 0 / 1 
        # q_vectors_new -> [P {SEP} H]  -> Linear(CLS)
        # ctx  --> H
        """
        ...
        if labels is not None:
            ce_loss = loss_fct(torch.squeeze(q_vectors_new, dim = 1), labels.float())

            loss = (1- alpha)*nll_loss + alpha*ce_loss
        else:
            loss = nll_loss
