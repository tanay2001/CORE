...
from dpr.models.biencoder_og import BiEncoder, BiEncoderNllLoss, BiEncoderBatch
...
class BiEncoderTrainer(object):
    ...
    def validate_nll(self) -> float:
        ...
        if cfg.local_rank in [-1, 0]:
            pass
            #wandb.log({'val_loss': total_loss, 'accuracy': correct_ratio})
        return total_loss
    ...
    def _train_epoch(
        ...
        for i, samples_batch in enumerate(train_data_iterator.iterate_ds_data(epoch=epoch)):
            ...
            if cfg.local_rank in [-1, 0]:
                wandb.log({'epoch': epoch, 'step': data_iteration,'train_loss': loss.item(), 'lr': lr})
    ...
    def _save_checkpoint(self, scheduler, epoch: int, offset: int) -> str:
        ...
        cp = os.path.join(cfg.output_dir, cfg.checkpoint_file_name)
...
def main(cfg: DictConfig):
    ...
    if cfg.train_datasets and len(cfg.train_datasets) > 0:
        trainer.run_train()
    if cfg.model_file and cfg.dev_datasets:
